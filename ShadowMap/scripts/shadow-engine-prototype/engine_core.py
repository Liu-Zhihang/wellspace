...
def canopy_to_gdf(raster: rasterio.io.DatasetReader, bbox: Mapping[str, float]) -> gpd.GeoDataFrame:
    ...
    mask = np.isfinite(data) & (data >= CANOPY_HEIGHT_THRESHOLD)
    if not np.any(mask):
        return gpd.GeoDataFrame(geometry=[], crs="EPSG:4326")

    labels = None
    component_heights: Dict[int, float] = {}
    try:
        ...
        labels, component_count = ndimage.label(mask.astype(np.uint8), structure=structure)
        if component_count > 0:
            for component_id in range(1, int(component_count) + 1):
                component_mask = labels == component_id
                if not np.any(component_mask):
                    continue
                component_heights[component_id] = float(np.nanmax(data[component_mask]))
            if not component_heights:
                labels = None
    except Exception:
        labels = None

    shape_source = labels.astype(np.int32) if labels is not None else mask.astype(np.uint8)
    fallback_height = float(np.nanmax(data[mask]))
    records: List[Dict[str, Any]] = []
    geoms: List[Any] = []
    for geom, val in rio_shapes(shape_source, mask=mask, transform=window_transform):
        if not geom:
            continue
        geoms.append(shape(geom))
        if labels is not None:
            height = component_heights.get(int(val), fallback_height)
        else:
            height = fallback_height
        records.append({"height": float(height)})
