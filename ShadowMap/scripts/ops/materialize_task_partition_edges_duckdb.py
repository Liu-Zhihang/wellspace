#!/usr/bin/env python3
"""Materialize task-to-candidate-partition edges for the national compute framework."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--task-graph-db", required=True, help="DuckDB from materialize_task_graph_duckdb.py")
    parser.add_argument("--partition-catalog-json", required=True, help="Partition catalog JSON path.")
    parser.add_argument(
        "--edge-table-name",
        default="task_partition_edges",
    ...
    parser.add_argument(
        ...
        type=float,
        default=400.0,
        help="Meters to expand each task bbox before partition matching.",
    )
    parser.add_argument(
        "--export-parquet",
        action="store_true",
...
def load_partition_catalog(path: Path) -> List[Dict[str, Any]]:
        ...
        rows.append(
            {
                ...
                "max_lon": max_lon,
                "max_lat": max_lat,
                "partition_size_deg": float(item.get("partitionSizeDeg", 0.0) or 0.0),
    ...
    if not rows:
        raise RuntimeError(f"Partition catalog is empty: {path}")
    return rows
...
def main() -> int:
    ...
    partition_fanout_table = f"{edge_table}_partition_fanout"

    if not db_path.exists():
        raise RuntimeError(f"Task-graph DuckDB not found: {db_path}")
    if not catalog_path.exists():
        raise RuntimeError(f"Partition catalog JSON not found: {catalog_path}")

    partition_rows = load_partition_catalog(catalog_path)
    ...
    try:
        ...
        con.execute("DROP TABLE IF EXISTS partition_catalog")
        con.execute("DROP TABLE IF EXISTS partition_catalog_raw")
        con.execute(f"DROP TABLE IF EXISTS {edge_table}")
        con.execute(f"DROP TABLE IF EXISTS {fanout_table}")
        con.execute(f"DROP TABLE IF EXISTS {partition_fanout_table}")

        con.execute(
            """
            CREATE TABLE partition_catalog_raw (
        ...
        execute_many(
            con,
            "INSERT INTO partition_catalog_raw VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                (
                    row["partition_id"],
                    row["parent_tile_id"],
        ...
        con.execute(
            """
            CREATE TABLE partition_catalog AS
            SELECT DISTINCT
              partition_id,
              parent_tile_id,
              min_lon,
        ...
        con.execute(
            ...
            SELECT
              ...
              pc.partition_id,
              pc.parent_tile_id,
              pc.partition_size_deg,
              pc.min_lon AS partition_min_lon,
              pc.min_lat AS partition_min_lat,
              pc.max_lon AS partition_max_lon,
              pc.max_lat AS partition_max_lat,
              pc.region,
              pc.description
            FROM expanded_tasks t
            JOIN partition_catalog pc
              ON pc.min_lon <= (t.east + t.pad_lon_deg)
             AND pc.max_lon >= (t.west - t.pad_lon_deg)
             AND pc.min_lat <= (t.north + t.pad_lat_deg)
             AND pc.max_lat >= (t.south - t.pad_lat_deg)
            """
        )
        con.execute(f"CREATE INDEX IF NOT EXISTS idx_{edge_table}_task_id ON {edge_table}(task_id)")
        con.execute(f"CREATE INDEX IF NOT EXISTS idx_{edge_table}_partition_id ON {edge_table}(partition_id)")
        con.execute(
            f"""
            ...
            FROM {edge_table}
            GROUP BY 1
            ORDER BY partition_count DESC, point_count DESC, task_id
            """
        )
        con.execute(
            f"""
            CREATE TABLE {partition_fanout_table} AS
            SELECT
              partition_id,
              parent_tile_id,
            ...
            FROM {edge_table}
            GROUP BY 1, 2
            ORDER BY task_count DESC, point_count DESC, partition_id
            """
        )
        ...
        edge_metrics = con.execute(
            ...
            SELECT
              ...
              count(DISTINCT partition_id)::BIGINT AS matched_partition_count,
              count(DISTINCT parent_tile_id)::BIGINT AS matched_parent_tile_count
            FROM {edge_table}
            """
        ).fetchone()
        task_metrics = con.execute(
            f"""
            SELECT
              avg(partition_count)::DOUBLE AS avg_partitions_per_task,
              max(partition_count)::BIGINT AS max_partitions_per_task,
              sum(CASE WHEN partition_count > 1 THEN 1 ELSE 0 END)::BIGINT AS multi_partition_task_count
            FROM {fanout_table}
            """
        ).fetchone()
        partition_metrics = con.execute(
            f"""
            SELECT
              avg(task_count)::DOUBLE AS avg_tasks_per_partition,
              max(task_count)::BIGINT AS max_tasks_per_partition
            FROM {partition_fanout_table}
            """
        ).fetchone()
        ...
        summary = {
            ...
            "partition_fanout_table_name": partition_fanout_table,
            "task_count": total_task_count,
            "matched_task_count": matched_task_count,
            "unmatched_task_count": total_task_count - matched_task_count,
            "partition_catalog_count": len(partition_rows),
            "matched_partition_count": int(edge_metrics[2] or 0),
            "matched_parent_tile_count": int(edge_metrics[3] or 0),
            "edge_count": int(edge_metrics[0] or 0),
            "avg_partitions_per_task": float(task_metrics[0] or 0.0),
            "max_partitions_per_task": int(task_metrics[1] or 0),
            "multi_partition_task_count": int(task_metrics[2] or 0),
            "avg_tasks_per_partition": float(partition_metrics[0] or 0.0),
            "max_tasks_per_partition": int(partition_metrics[1] or 0),
        }
        summary["multi_partition_task_fraction"] = (
            float(summary["multi_partition_task_count"]) / float(matched_task_count)
        ...
        if args.export_parquet:
            ...
            con.execute(
                f"COPY {partition_fanout_table} TO {sql_quote(str(parquet_dir / f'{partition_fanout_table}.parquet'))} (FORMAT PARQUET)"
            )
