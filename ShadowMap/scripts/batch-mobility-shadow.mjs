 ...
 *
 * Optional:
 *   --backend, --weather, --canopy, --include-canopy, --concurrency, --force, --buckets-file, --target-file, --targets-file
 */
...
const config = {
  ...
  targetsFile: args['targets-file'] ?? args['targetsFile'],
};
...
const normalizeConfig = () => {
  ...
  if (config.targetsFile) {
    config.targetsFile = path.resolve(config.targetsFile);
  }
  if (config.targetFile && config.targetsFile) {
    throw new Error('Use either --target-file or --targets-file (not both)');
  }
};
...
const readTargetsFromFile = async (filePath, inputRoot) => {
  const text = await fs.readFile(filePath, 'utf-8');
  const skipMissing = parseBool(process.env.MOBILITY_SKIP_MISSING_TARGETS, false);
  const unique = [];
  const seen = new Set();
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    const candidate = path.isAbsolute(line) ? path.resolve(line) : path.resolve(inputRoot, line);
    if (path.extname(candidate).toLowerCase() !== '.csv') continue;

    const relative = path.relative(inputRoot, candidate);
    if (relative.startsWith('..') || path.isAbsolute(relative)) {
      throw new Error(`target_outside_input_root: candidate=${candidate} input_root=${inputRoot}`);
    }
    ...
    try {
      ...
      if (skipMissing) {
        console.warn(`[Targets] skip missing file: ${candidate}`);
        continue;
      }
      throw new Error(`targets_missing: ${candidate}`);
    }
...
const main = async () => {
  ...
  let files = config.targetsFile
    ? await readTargetsFromFile(config.targetsFile, config.inputRoot)
    : await listCsvFiles(config.inputRoot);
